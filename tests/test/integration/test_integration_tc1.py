...
class TestPartialUpdateBooking(object):
    @pytest.fixture()
    ...
    @allure.title("Test Partial Update (PATCH)")
    @allure.description("Verify that Partial Update with the booking ID and Token is working.")
    def test_update_booking_id_token(self, create_token, get_booking):
        ...
        verify_response_key(key=response.json()["firstname"], expected_data="Drithi")